...
where 
	 A.ABSENCE_YN = 'Y'
order by 
	  concat(if(if(substring(A.STUDENT_SSN,1,1) = '0','2' ,substring(A.STUDENT_SSN,1,1)) ='9','0','2'),  substring(A.STUDENT_SSN,2,5)) desc
;
...
select  
	 ...
	 ,A.ENTRANCE_DATE
from tb_student A
where (A.STUDENT_ADDRESS like '%강원%' or A.STUDENT_ADDRESS like '%경기%')
and substring(date_format(ENTRANCE_DATE,'%Y'),1,4) >= '2020'
order by A.STUDENT_NAME
;
...
select A.STUDENT_NO, A.point
from tb_grade A
join tb_class B on (B.CLASS_NO = A.CLASS_NO)
where A.CLASS_NO = 'C3118100'
and A.TERM_NO = '202202'
...
join tb_professor B on (B.PROFESSOR_NO = A.PROFESSOR_NO)
join tb_class C on (C.ClASS_NO = A.ClASS_NO)
order by B.PROFESSOR_NAME,C.CLASS_NAME;
...
join tb_professor B on (B.PROFESSOR_NO = A.PROFESSOR_NO)
join tb_class C on (C.ClASS_NO = A.ClASS_NO)
where B.DEPARTMENT_NO in (select DEPARTMENT_NO
						  from tb_department
						  where CATEGORY = '인문사회'
						  )
order by B.PROFESSOR_NAME,C.CLASS_NAME;
...
select 
	   C.STUDENT_NO as "학번"
	  ,C.STUDENT_NAME as "학생이름"
	  ,format(AVG(A.POINT),1) as "학점"
from tb_grade A
join tb_class B on (B.CLASS_NO = A.CLASS_NO)
join tb_student C on (C.STUDENT_NO = A.STUDENT_NO)
where B.DEPARTMENT_NO in (select DEPARTMENT_NO
						  from tb_department
...
group by  C.STUDENT_NO
	  	 ,C.STUDENT_NAME
order by format(AVG(A.POINT),1) DESC
...
select 
 	  B.DEPARTMENT_NAME as "학과이름"
 	 ,A.STUDENT_NAME as "학생이름"
 	 ,C.PROFESSOR_NAME as "박태환"
from  tb_student A 
join  tb_department B on(B.DEPARTMENT_NO = A.DEPARTMENT_NO)
...
select 
 	  ...
 	  ,A.TERM_NO
from  tb_grade A
join  tb_student B on(B.STUDENT_NO = A.STUDENT_NO)
join  tb_department C on(C.DEPARTMENT_NO = B.DEPARTMENT_NO)
join  tb_class D on(D.CLASS_NO = A.CLASS_NO)
where A.CLASS_NO = (select CLASS_NO
					from tb_class
					where CLASS_NAME = '인간관계론'
					)
and A.TERM_NO like '2022%'
;
...
from 
	 (
			select A.CLASS_NO,B.DEPARTMENT_NO,A.CLASS_NAME,A.CLASS_TYPE,B.OPEN_YN
			from tb_class A
			join tb_department B on(B.DEPARTMENT_NO = A.DEPARTMENT_NO)
			where B.CATEGORY = '예체능'

	  )  B

select B.CLASS_NAME,B.DEPARTMENT_NAME
from (
		select 
		 	  A.PROFESSOR_NO
		 	 ,A.CLASS_NO
...
right join (
			select A.CLASS_NO,B.DEPARTMENT_NO,A.CLASS_NAME,B.DEPARTMENT_NAME
			from tb_class A
			join tb_department B on(B.DEPARTMENT_NO = A.DEPARTMENT_NO)
			where B.CATEGORY = '예체능'

	  	  )  B
on (B.CLASS_NO = A.CLASS_NO)
where A.PROFESSOR_NO is null
order by  B.DEPARTMENT_NAME,B.CLASS_NAME asc;
...
select 
	  A.STUDENT_NAME
	 ,if(C.PROFESSOR_NAME is null,'지도교수 미지정',C.PROFESSOR_NAME) as "지도교수"
from tb_student A
left join  tb_department B on(B.DEPARTMENT_NO = A.DEPARTMENT_NO)
left join  tb_professor C on(C.PROFESSOR_NO = A.COACH_PROFESSOR_NO)
where B.DEPARTMENT_NAME = '서반아어학과'
order by substring(date_format(ENTRANCE_DATE,'%Y'),1,4) ASC;
...
group by B.STUDENT_NO 
	  ,B.STUDENT_NAME
	  ,C.DEPARTMENT_NAME
having FORMAT(AVG(A.POINT),2) >= 4.0
order by AVG(A.POINT) DESC
;
...
from tb_student  A
join tb_department B on (B.DEPARTMENT_NO = A.DEPARTMENT_NO)
where B.DEPARTMENT_NO in (select B.DEPARTMENT_NO
							from tb_student  A
							join tb_department B on (B.DEPARTMENT_NO = A.DEPARTMENT_NO)
...
from
	(
	select B.STUDENT_NO as STUDENT_NO
		  ,B.STUDENT_NAME as STUDENT_NAME
		  ,AVG(A.POINT) as POINT
	from tb_grade A
	join tb_student B on(b.student_no = A.student_no)
	join tb_department C on (C.DEPARTMENT_NO = B.DEPARTMENT_NO)
	where C.DEPARTMENT_NAME = '국어국문학과'
	group by B.STUDENT_NO,B.STUDENT_NAME
	) A
where A.POINT = (select max(POINT)
				 from
					(
					select B.STUDENT_NO as STUDENT_NO
						  ,B.STUDENT_NAME as STUDENT_NAME
...
select 
	   C.DEPARTMENT_NAME   as "계열학과명"
	  ,round(AVG(A.POINT),1) as "전공평점"
from tb_grade A
join tb_class B on(b.class_no = A.class_no)
join tb_department C on (C.DEPARTMENT_NO = B.DEPARTMENT_NO)
where C.CATEGORY = '자연과학'
