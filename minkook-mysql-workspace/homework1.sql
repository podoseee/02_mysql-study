...
select 
	  ...
	  ,B.book_name  
from tbl_order A
join tbl_book B on(B.book_id = A.book_id)
where date_format(A.ordered_at,'%Y%m%d') >= '20200705'
and date_format(A.ordered_at,'%Y%m%d') <= '20200709'
;
...
select 
	   cust_name as "고객명"
from tbl_order A
right join tbl_customer B on(B.cust_id = A.cust_id)
...
from tbl_order A
join tbl_book B on(B.book_id = A.book_id)
join tbl_customer C on(C.cust_id = A.cust_id)
where date_format(A.ordered_at,'%Y%m%d') >= '20200704'
and date_format(A.ordered_at,'%Y%m%d') <= '20200707'
...
select 
	   ...
	   ,C.cust_name as "구매자"
	   ,B.book_name as "책이름"
	   ,B.price * A.amount as "총구매액"
	   ,A.ordered_at as "주문일자"
from tbl_order A
join tbl_book B on(B.book_id = A.book_id)
join tbl_customer C on(C.cust_id = A.cust_id)
where A.order_id not in(select 
							  A.order_id 
						from tbl_order A
						where date_format(A.ordered_at,'%Y%m%d') >= '20200704'
						and date_format(A.ordered_at,'%Y%m%d') <= '20200707'
...
select 
	  ...
	  , B.book_name
from tbl_order A
right join tbl_book B on(B.book_id = A.book_id)
where A.order_id is null
;
...
left join tbl_customer C on(C.cust_id = A.cust_id)
where A.price = (select 
					   max(price)
				 from tbl_book 
				 )
;
...
select 
	  C.cust_name as "고객명"
     ,if(sum(A.price) is null,0,sum(A.price)) as "총구매액"
     ,count(A.order_id) as "구매횟수"
from
(
select    A.order_id
		 ,A.cust_id
		 ,A.book_id
		 ,B.price* A.amount as price
	from tbl_order A
	join tbl_book B on(B.book_id = A.book_id)
) A
right  join tbl_customer C on(C.cust_id = A.cust_id)
group by C.cust_name
