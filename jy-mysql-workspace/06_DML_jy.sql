...
/*
    ...
    2. 데이터 조작어 - 데이터를 조작하기 위해 사용되는 언어
    3. 종류
       1) 삽입 : INSERT
       2) 수정 : UPDATE
       3) 삭제 : DELETE
...
/*
    ## RAPLACE ##
    INSERT시 PRIMARY KEY 또는 UNION KEY가 중복될 여지가 있을 경우
    REPLACE를 통해 중복된 데이터를 덮어씌울 수 있음
    정확히는 삭제되었다가 다시 삽입되는 구조
*/
-- INSERT INTO tbl_menu VALUES(17, '참기름소주', 5000, 10, 'Y'); -- 중복키가 존재하기 때문에 오류 발생
REPLACE INTO tbl_menu VALUES(17, '참기름소주', 5000, 10, 'Y'); -- 중복키가 존재하므로 수정
REPLACE /*INTO*/ tbl_menu VALUES(40, '참치액젓소주', 5000, 10, 'Y'); 
...
/*
    ...
    3. 문법
       ...
       SET 컬럼명 = 바꿀값,
           컬럼명 = 바꿀값,
           ...
       [WHERE 조건]; -- 조건절이 업으면 전체 행 대상으로 다 수정되므로 유의할 것!!
*/

-- 매생이초콜릿 메뉴의 카테고리번호를 10으로 변경
UPDATE
    tbl_menu
SET
    category_code = 10;
...
-- 에스프레소번데기 메뉴의 카테고리번호를 10으로, 가격을 3000원으로, 메뉴명을 에스프랫소번데기탕으로 변경
...
SET
    menu_price = (SELECT menu_price
                    FROM tbl_menu
                   WHERE menu_name = '우럭스무디') -- oracle 에선 가능하나 mysql에서는 자기 자신 테이블의 데이터 사용시 1093 에러 발생 
WHERE
    menu_code = 22;
