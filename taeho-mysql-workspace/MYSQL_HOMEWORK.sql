...
INSERT INTO tbl_book (book_id, book_name, publisher, price) VALUES
(1, '축구의 역사', '굿스포츠', 7000),
...
INSERT INTO tbl_customer (cust_id, cust_name, cust_addr, cust_tel) VALUES
...
INSERT INTO tbl_order (order_id, cust_id, book_id, amount, ordered_at) VALUES
...
SELECT * FROM tbl_book WHERE book_name LIKE('%올림픽%');

-- 8. 가격이 가장 비싼 책을 조회하시오.
SELECT * FROM tbl_book ORDER BY price DESC LIMIT 1;
...
FROM tbl_customer
WHERE cust_id = (
    SELECT cust_id FROM tbl_customer WHERE cust_id NOT IN (
...
SELECT 
    (SELECT cust_name FROM tbl_customer WHERE cust_id = o.cust_id) AS '고객명',
    (SELECT book_name FROM tbl_book WHERE book_id = o.book_id) AS '책이름',
    o.ordered_at AS '주문일자'
FROM tbl_order o
...
WHERE book_id NOT IN (
    SELECT DISTINCT book_id FROM tbl_order
    );
...
SELECT IFNULL(c.cust_name, NULL) AS '고객이름',
    b.book_name AS '책이름',
    b.price AS '가격'
FROM tbl_book b
...
SELECT 
    c.cust_name AS '고객명',
    (SELECT count(*) FROM tbl_order o WHERE c.cust_id = o.cust_Id) AS '구매도서수'
FROM tbl_customer c
...
SELECT
    ...
    IFNULL(COUNT(o.order_id), 0) AS '구매횟수'
FROM tbl_customer c
LEFT JOIN tbl_order o ON c.cust_id = o.cust_id
LEFT JOIN tbl_book b ON  b.book_id = o.book_id
GROUP BY c.cust_id, c.cust_name
