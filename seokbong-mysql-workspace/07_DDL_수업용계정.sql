...
SELECT*FROM information_schema.TABLE_CCPNSTRAINTS WHERE table_name = 'table_like';
...
INSERT
    tbl_user
VALUES
    (null, 'user16', 'pass16', '광개토', '남', 80); -- 16
...
CREATE TABLE tbl_user(
    ...
    user_pwd VARCHAR(255) NOT NULl,
    user_name VARCHAR(255) NOT NULl,
    grade_code INT, -- 오로지 tbl_user_grade 테이블이 grade_id 컬러값만 들어올 수 있게 허용
...
ALTER TABLE tbl_user
    ADD age INT CHECK(age>=19) DEFAULT 19 COMMENT '나이' AFTER phone;-- 데이터로는 DEFAULT값이 들어감
...
ALTER TABlE tbl_user MODIFY user_no VARCHAR(255); -- o (데이터가 있어도 문자형으로 허용할만한 데잍터이므로), PK유지됨
...
DROP TABLE IF EXISTS tbl_user_grade; -- 참조되는 부모테이블같은 경우 삭제 제한됨(자식 테이블 삭제)
...
/*
    ## TRUNCATE ##
    테이블 내의 데이터만 삭제하는 구문
    즉, 테이블 구조는 남기고 모든 행만 삭제하는 명령어
    
    * DELETE VS TRUNCATE
    - DELETE는 조건제시 가능     => 일부행만 삭제도 가능
      TRUNCATE는 조건제시 불가능 => 전체행만 삭제 가능
    - DELEETE는 삭제시 내부적으로 로그를 남기므로 ROLLBACK이 가능하나 속도가 느림
      TRUNCATE는 삭제시 내부적으로 로그를 남기지 않아 속도는 바르나 ROLLBACK이 안됨
*/
