...
/*
    ## 컬럼 별칭(Alias) ##
    조회되는 ResaultSet의 컬럼명(헤더) 부분을 내가 원하는 별칭으로 조회 가능
...
/*
    ## WHERE절 ##
    1. 테이블의데이터 중 원하는 데이터만 선택적으로 조회할 때 사용 (일부 행)
    2.WHERE 조건식
...
FROM
    tbl_category
WHERE
--    ref_category_code = NULL;  (=은 실제로 존재하는 값만 비교 가능. 즉, *NULL은 = 으로 비교 불가*)
...
FROM
    tbl_category
WHERE
--    ref_category_code = NULL;  (!= 또한 실제로 존재하는 값만 비교 가능. 즉, *NULL은 != 으로 비교 불가*)
