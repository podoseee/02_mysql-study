...
-- 카테고리별 메뉴의 개수, 평규낙격, 최대가격, 최소가격 조회
SELECT
    category_code
  , count(*) AS 메뉴개수
...
/*
    ## 집합연산자(Set Operator) ##
    여러개의 질의 결과를 칼럼끼리 연결하여 하나의 ResultSet을 만드는 방식
...
WHERE
    menu_price < 9000
ORDER BY menu_name; -- ORDER BY절은 마지막 쿼리에서 한번만 작성 가능
