...
SELECT category_code FROM tbl_menu WHERE menu_name = '열무김치라떼'; -- 8 sub

-- 2) tbl_category로 부터 해당 카테고리번호의 카테고리명  (메인쿼리)
...
FROM
    tbl_category c
        JOIN tbl_menu m ON m.category_code = c.category_code
WHERE
    menu_name = '열무김치라떼';
...
-- 2) tbl_menu로부터 카테고리번호 4인 음식 조회
-- SELECT menu_code, menu_name FROM
...
-- 실습. 생갈치쉐이크와 같은 가격이고 같은 카태고리인 메뉴 조회 (메뉴코드, 메뉴명, 가격, 카테고리번호)
SELECT
    menu_code
  , menu_name
  , menu_price
...
WHERE
    menu_price = (SELECT menu_price
                    FROM tbl_menu
                   WHERE menu_name = '생갈치쉐이크') -- 6000
AND
    category_code = (SELECT category_code
                       FROM tbl_menu
                      WHERE menu_name = '생갈치쉐이크'); -- 10
...
/*
    ...
    2. 서브쿼리를 가지고 일방 비교연산자만을 가지고 비교 불가
    3. 사용가능한 연산자
       1) IN (값1, 값2, ..)
       ...
       3) > ALL (값1, 값2, ..)
          값1보다 크고(AND), 값2보다 크고(AND), ... 이어야만 참
          즉, 여러개의 값들보다 커야만 참
...
-- 실습. 한식메뉴(4번 카테고리)들보다 비싼 메뉴 조회(모든 컬럼)
SELECT
    *
FROM
    tbl_menu
...
/*
    ...
    2. 메인쿼리 테이블의 레코드(행)에 따라 서브쿼리의결과도 매번 달라짐
