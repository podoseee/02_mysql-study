/*
    ...
    1. 어떤 컬럼에 인덱스를 만들면 좋을까?
       - 중복된 데이터값들이 없는 고유한 데이터값을 가지는 컬럼에 만드는게 제이 좋음
...
/*
    ...
    * type 종류 (성능 우선순위로 나열)
    ...
    - ref : 조인의 순서와 인덱스의 종류 상관없이 equal 조건으로 검색
    - unique_subquery : IN(sub-query) 형태의 조건에서 반환 값에 중복 없음
    - index_subquery : unique subquery와 비슷하지만 반환 값에 중복 있음
...
EXPLAIN SELECT * FROM tbl_menu WHERE category_code = 4; -- ref (index로 등록되어 있는 fk컬럼으로 equal 조건으로 조회)
...
SELECT *
FROM phone
-- WHERE phone_name = 'iphone16rpo'; -- ALL
-- WHERE phone_price = 30000000;
