...
FROM
    employee
WHERE
#   연봉 * 12 >= 50000000 WHERE절에서는 사용 불가함 ? 왜 ?  WHERE절은 SELECT절보다 먼저 실행되기 때문
    SALARY * 12 >= 50000000;
;
...
FROM
    employee
WHERE
    CHAR_LENGTH(SUBSTRING_INDEX(EMAIL,'_',1)) = 4 # 이메일에서 _ 문자 왼쪽 첫번째 부분을 가져옴
AND (DEPT_CODE ='D5' OR DEPT_CODE = 'D9')
                # AND (DATEDIFF('1990-01-01', HIRE_DATE) <= 0 OR DATEDIFF('2001-12-31', HIRE_DATE) >= 0)
...
FROM
    employee
WHERE
    MANAGER_ID IS NULL # MANAGER_ID = NULL은 알 수 없는 값으로 어떤값과 비교해도 FALSE임
;
