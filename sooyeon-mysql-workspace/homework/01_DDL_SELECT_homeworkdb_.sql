...
GRANT ALL PRIVILEGES ON homeworkdb.* TO 'sotogito'@'%';
...
CREATE TABLE IF NOT EXISTS tbl_book(
    book_id     INT             PRIMARY KEY  AUTO_INCREMENT     COMMENT'책번호',
...
CREATE TABLE IF NOT EXISTS tbl_customer(
    cust_id     INT             PRIMARY KEY  AUTO_INCREMENT     COMMENT'고객번호',
...
CREATE TABLE IF NOT EXISTS tbl_order(
    order_id    INT     PRIMARY KEY  NOT NULL  AUTO_INCREMENT       COMMENT"주문번호",
...
INSERT INTO
    tbl_customer(cust_name,cust_addr,cust_tel)
VALUES
    ("박지성","영국","000-000-0000"),
...
INSERT INTO
    tbl_order(cust_id,book_id,amount,ordered_at)
VALUES
    (1000,1,1,'2020-07-01'),
    (1000,3,2,'2020-07-03'),
    (1001,5,1,'2020-07-03'),
...
SELECT
    publisher AS 출판사
    ,COALESCE(COUNT(amount),0) AS 판매된책수 # 만약에 판매된 책의 누적 수량이라면 SUM(amount)
FROM
    tbl_book b
        LEFT JOIN tbl_order o ON o.book_id = b.book_id
GROUP BY
...
FROM
    tbl_order o
        JOIN tbl_book b ON b.book_id = o.book_id
        RIGHT JOIN tbl_customer c ON c.cust_id = o.cust_id
       -- LEFT JOIN tbl_book b ON b.book_id = o.book_id     #조인시키는 순서도 중요함
GROUP BY
    cust_name
;
...
HAVING
    총구매액 <> (
                    SELECT
                        SUM(price * amount) AS 최고구매금액
                    FROM
                        tbl_order o
                            JOIN tbl_book b ON b.book_id = o.book_id
                    GROUP BY cust_id
                    ORDER BY 최고구매금액 DESC
                    LIMIT 1
                ) 
ORDER BY
    총구매액 DESC
