...
SELECT
    emp_name AS 직원명
  , hire_date AS 입사일
  , DAY(LAST_DAY(hire_date)) - DAY(hire_date) + 1 AS '입사한 달의 근무일수'
FROM
    employee;
...
*/
SELECT
  SUM((salary + salary * COALESCE(bonus,0)) * 12) AS '연봉'
FROM
    employee
WHERE
    dept_code = 'D5';
...
*/
SELECT
    SUM(IF(SUBSTRING(emp_no, 8, 1) IN ('1', '3'), salary, 0)) AS '남사원 급여 합계',
    SUM(IF(SUBSTRING(emp_no, 8, 1) IN ('2', '4'), salary, 0)) AS '여사원 급여 합계'
FROM
    employee;
