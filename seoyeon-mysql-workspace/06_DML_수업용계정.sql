...
REPLACE /*INTO*/ tbl_menu VALUES(17, '참치액젓소주', 5000, 10, 'Y'); -- 중복키가 존재하지 않으므로 삽입
...
-- '에스프레소번데기' 메뉴의 카데고리번호를 10으로, 가격을 3000원으로, 메뉴명을 '에스프레소번데기탕'으로 변경
UPDATE
    tbl_menu
SET
    category_code = 10,
...
SET
    menu_price = (SELECT menu_price
                    FROM tbl_menu
                   WHERE menu_name = '우럭스무디') -- oracle에선 가능하나 mysql에서는 자기 자신 테이브르이 데이터 사용시 1093 에러 발생
WHERE
    menu_code = 22; 
