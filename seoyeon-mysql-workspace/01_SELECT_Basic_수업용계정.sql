/*
    ...
    1. DQL (Data Query Language) : 데이터 질의 언어 (데이터 조회용)
        - SELECT (데이터 조회)
    2. DML (Data Manipulation Language) : 데이터 조작 언어 (데이터 삽입, 수정, 삭제)
        - INSERT (데이터 삽입)
        - UPDATE (데이터 수정)
        - DELETE (데이터 삭제)
...
FROM
    tbl_menu
WHERE
--    menu_price >= 10000 AND menu_price <= 25000; -- 13 row
...
/*
    ...
    2. 오름차순 정렬(ASC)과 내림차순 정렬(DESC) 존재
    3. 표현법
...
--    ref_category_code DESC                                -- 내림차순 정렬시 기본적으로 NULL이 하단에 위치함
    ref_category_doce IS NULL DESC, ref_category_code DESC  -- 내림차순 정렬시 NULL을 상단에 위치시키고자 할 경우
;
