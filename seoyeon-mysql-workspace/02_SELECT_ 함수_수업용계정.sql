/*
    ...
    5. 종류
        1) 단일행함수 : 각 행마다 반복적으로 작업을 수행하여 결과를 반환 (즉, N개의 값을 읽어들여 N개의 결과 반환)
            ...
            - 날짜/시간처리함수
            - 기타함수
        2) 그룹함수   : 여러행들이 그룹으로 형성되어 적용됨. 그룹 1개의 결과 반환 (즉, N개의 값을 읽어들여 1개의 결과 반환)
*/
...
/*
    ...
    2. BIT_LENGTH(문자열)  : 해당 문자열의 할당된 비트 크기 반환 (LENTH결과*8) (1Byte = 8Bit)
...
SELECT
    ELT(2, '피아노', '기타', '플룻')               -- 기타
  , FIELD('피아노', '피아노', '기타', '플룻')      -- 1
  , FIND_IN_SET('기타', '피아노,기타,플룻');       -- 2
...
*/
SELECT
    FORMAT(12501035910985.31498103, 3);
...
-- 쥬스가 포함되어있는 메뉴이름을 JUICE로 변경해서 조회
SELECT
    menu_name
...
SELECT
    RPAD(SUBSTRING('030417-4321234', 1, 8), 14, '*');
    
SELECT
    SUBSTRING_INDEX('17olivia@naver.com', '@', 1)
  , SUBSTRING_INDEX('17olivia@naver.com', '.', 1)
  , SUBSTRING_INDEX('17olivia@naver.com', '.', -1);
...
/*
    ...
    3. TRUCATE(숫자, 자리수) : 해당 숫자의 버림값 반환. 소수점이하 자리수는 필수
...
/*
    ## POW, SQRT ##
    1. POW(숫자1, 숫자2) : 숫자1을 숫자2 거듭제곱한 결과 반환
...
/*
    ## SIGN ##
    SIGN(숫자) : 해당 숫자가 양수면 1, 음수면 -1, 0이면 0을 반환
*/
SELECT
    SIGN(10.1)
  , SIGN(0)                        
...
SELECT
  ...
  */
  ...
  /*
    ...
    날짜 및 시간에서 연, 얼, 일, 시, 분, 초, 밀리초를 추출해서 반환
  */
  SELECT
    YEAR(CURDATE())
  , MONTH(CURDATE())
...
/*
    ## EXTRACT ##
    EXTRACT(UNIT FROM 날짜및시간) : 날짜및시간 으로부터 UNIT에 해당하는 값 추출해서 반환
...
/*
    ## NULL 처리 함수 ##
    IFNULL(값, NULL일 때 대체값) : 값이 NULL일 경우 대체값 반환, NULL이 아닐경우 기존값 반환
    ISNULL(값) : 값이 NULL일 경우 1(True) 반환
    COALESCE(값1, 값2, ..) : 나열된 값들 중 NULL이 아닌 첫 번째 값을 반환
*/
SELECT
    IFNULL(NULL, 'Hello Future')
  , IFNULL('ㅋㅋㅋ', '기다렸어 어서 와');
...
SELECT
    COALESCE(NULL, NULL, '이서연', NULL, '이태용');
...
-- AVG(숫자타입) : 컬럼값들의 평균을 구해서 반환 (SUM과 COUNT를 가지고 연산 내부적으로 진행)
SELECT
    AVG(menu_price)
FROM
    tbl_menu;
