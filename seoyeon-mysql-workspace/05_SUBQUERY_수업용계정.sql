...
-- 2) tbl_category로부터 해당 카테고리번호의 카테고리명 조회 (메인쿼리)
...
-- 2) tbl_menu로부터 해당 카테고리번호의 메뉴들 조회
SELECT
    menu_code
    , menu_name
    , menu_price
...
FROM
    tbl_menu
        JOIN tbl_category USING(category_name)
WHERE
...
-- 실습. 한식메뉴(4번카테고리)들보다 비싼 메뉴 조회 (모든칼럼)
SELECT
    *
FROM
    tbl_menu
...
use empdb;
-- 실습. 영업부에 해당하는 사원들의 사번, 사원명, 부서명, 급여 조회
SELECT
    emp_id
  , emp_name
  , dept_title
...
SELECT
  ...
  , (SELECT category_name
       FROM tbl_category
      WHERE category_code = m.category_code) AS 카테고리명 -- 일치하는 데이터가 없으면 NULL로 반환
FROM 
    tbl_menu m;
    
-- 전체 메뉴 조회시 각메뉴별로 몇 건 주문되었는지
SELECT
    m.menu_code
  , m.menu_name
  , (SELECT COUNT(*) FROM tbl_order_menu WHERE menu_code = m.menu_code) "주문 건수"
FROM
    tbl_menu m;
...
--  , e.manager_id
  , IFNULL((SELECT emp_name
              FROM employee
             WHERE emp_id = e.manager_id), '없음') AS 사수명
FROM
    employee e;
...
SELECT
  ...
  , (SELECT COUNT(*)
       FROM employee
      WHERE dept_code = e.dept_code) AS 부서원수
  , ROUND((SELECT AVG(salary)
       FROM employee
      WHERE dept_code = e.dept_code)) AS 부서평균급여
FROM
    employee e;
...
SELECT
    MIN(메뉴개수)
    ,MAX(메뉴개수) # 이름을 붙여 커스텀 테이블 중 값이 가장 큰 값을 구함
FROM (SELECT
        category_code AS 카테고리
        ,COUNT(*) AS 메뉴개수# 서브쿼리로 바로 하나의 테이블을 생성하고 그것을 FROM절로 넘겨 활용함
    FROM tbl_menu
    GROUP BY category_code) AS menu_count;
;
WITH menu_count AS(
    SELECT
        category_code AS 카테고리
