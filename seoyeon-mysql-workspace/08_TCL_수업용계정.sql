...
-- 작업1. 이서연이 이태용에게 100만원 이체
UPDATE ssg_bank SET balance = balance - 1000000 WHERE id = 1231; -- 이서연 돈 차감
UPDATE ssg_bank SET balance = balance + 1000000 WHERE id = 456;  -- 이태용 돈 증가
COMMIT;

-- 작업2. 이태용이 이서연에게 5000만원 이체
UPDATE ssg_bank SET balance = balance - 50000000 WHERE id = 456; -- 이태용 돈 차감
UPDATE ssg_bank SET balance = balance + 50000 WHERE id = 1231;   -- 이서연 돈 증가
-- 문제발생
ROLLBACK;
...
-- 작업1. 1번메뉴 2개, 2번메뉴 1개 주문완료 (총 주문가격 14000)
