...
WHERE l.local_name LIKE 'ASIA%' AND j.job_name = '대리';
...
FROM
    ...
    LEFT JOIN job j ON e.job_code = j.job_code
    LEFT JOIN location l ON l.local_code = d.location_id
WHERE e.emp_name LIKE '전%' AND e.emp_no LIKE '%-2%' OR '%-4%';
...
FROM
    employee e
    JOIN dept d ON d.dept_id = e.dept_code
    JOIN location l ON l.local_code = d.location_id;
...
FROM
    employee e1
    JOIN employee e2 ON e2.dept_code = e1.dept_code AND e2.emp_name <> e1.emp_name
