...
(1, '축구의 역사', '굿스포츠', 7000),  -- 출판사 pub_no는 기본값으로 설정
(2, '축구 아는 여자', '나이스북', 13000),
(3, '축구의 이해', '대한미디어', 22000),
(4, '골프 바이블', '대한미디어', 35000),
(5, '피겨 교본', '굿스포츠', 6000),
...
FROM 
    tbl_book b
WHERE 
    b.book_id NOT IN (SELECT o.book_id FROM tbl_order o);
...
SELECT
  ...
  , COUNT(publisher)
FROM
    (SELECT cust_name, b.publisher publisher
        FROM tbl_customer c
        JOIN tbl_order o ON o.cust_id = c.cust_id
        JOIN tbl_book b ON b.book_id = o.book_id ) result
GROUP BY
    cust_name
HAVING
    cust_name = '박지성';
...
SELECT
  ...
  , SUM(price * amount)
FROM
    (SELECT cust_name, b.price price, o.amount amount
        FROM tbl_customer c
        JOIN tbl_order o ON o.cust_id = c.cust_id
        JOIN tbl_book b ON b.book_id = o.book_id ) result
GROUP BY
...
SELECT
    c.cust_name,
    IFNULL(SUM(b.price * o.amount), 0) AS total_amount,
    IFNULL(COUNT(o.order_id), 0) AS purchase_count
FROM
    tbl_customer c
