...
FROM
    tbl_user
WHERE
    LEFT(user_regdate, 4) >= 2010;
-- 4. 사용자번호와 연락처1, 연락처2를 연결하여 조회하시오. 연락처가 없는 경우 NULL 대신 'None'으로 조회하시오.
SELECT
    CONCAT(user_no, IFNULL(user_mobile1, 'None'),IFNULL(user_mobile2, 'None'))
FROM
    tbl_user;
...
FROM
    tbl_product p
    JOIN tbl_buy b USING(prod_code)
GROUP BY
    prod_category;
-- 9. 아이디별 구매횟수를 조회하시오.
SELECT
...
SELECT
    u.user_id,
    COALESCE(COUNT(b.user_no), 0) AS purchase_count
FROM
    tbl_user u
...
SELECT
    prod_name
  , IFNULL(COUNT(b.prod_code), 0)
FROM
    tbl_product p
    LEFT JOIN tbl_buy b USING(prod_code)
GROUP BY prod_name;
...
SELECT
    ...
    (SELECT COUNT(*)
     FROM tbl_buy
     WHERE user_no = u.user_no) AS purchase_count
FROM
    tbl_user u
HAVING
    purchase_count >= 2;
...
SELECT * FROM tbl_user; -- `user_no`, `user_id`, `user_name`, `user_year`, `user_addr`, `user_mobile1`, `user_mobile2`, `user_regdate`
SELECT * FROM tbl_product; -- prod_code prod_name prod_category prod_price
SELECT * FROM tbl_buy; -- `buy_no`, `user_no`, `prod_code`, `buy_amount`
UPDATE
    tbl_user
SET 
    user_mobile1 =  '010'
...
DELETE
FROM tbl_buy
WHERE buy_no = (SELECT buy_no FROM (
                    SELECT MAX(buy_no) AS buy_no FROM tbl_buy) tmp);
