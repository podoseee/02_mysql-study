...
SELECT
    CASE
        WHEN user_mobile1 IS NULL OR user_mobile2 IS NULL THEN 'None'
        ELSE CONCAT(user_no + ' / ' + user_mobile1 + ' / ' + user_mobile2)
    END
FROM
    tbl_user;
...
select
    ...
    ,COUNT(*)
from
    tbl_user u LEFT JOIN tbl_buy b
ON u.user_no = b.user_no
...
FROM 
    tbl_product p
LEFT JOIN 
    tbl_buy b ON p.prod_code = b.prod_code
GROUP BY 
...
delete
from tbl_buy
where buy_no = (select max(buy_no) from tbl_buy);
...
--     사용자번호가 5인 사용자의 구매 내역을 먼저 삭제한 뒤 진행하시오.   -> 외래키 제약조건떄문임
select * from tbl_buy;
delete
from tbl_buy
where user_no = 5;
