...
SELECT 
    e.emp_id AS 사원번호
    ,e.emp_name AS 성명
    ,CONCAT(LEFT(e.emp_no, 8), '******')AS 주민번호
    ,(e.salary +e.salary * e.bonus)*12 AS 연봉
FROM
    EMPLOYEE e
WHERE
    SUBSTRING(e.emp_no, 8, 1) IN ('1', '3'); 
...
SELECT
    ...
    SUM(salary + (salary*bonus)*12) AS "연봉"
FROM
    EMPLOYEE
WHERE
    dept_code = 'D5';
...
*/
SELECT 
    SUM(CASE WHEN SUBSTRING(e.emp_no, 8, 1) IN ('1', '3') THEN salary ELSE 0 END) AS 남사원_급여_합계,
    SUM(CASE WHEN SUBSTRING(e.emp_no, 8, 1) IN ('2', '4') THEN salary  ELSE 0 END) AS 여사원_급여_합계
FROM 
    EMPLOYEE e;
