...
SELECT * FROM employee;
SELECT
    LEFT(hire_date, 4) 입사년
    , SUM(IF(job_code != 'J1', 1, 0)) 인원수
FROM
    employee
GROUP BY
    LEFT(hire_date, 4)
HAVING
    SUM(IF(job_code != 'J1', 1, 0)) != 0
ORDER BY
    LEFT(hire_date, 4);
...
SELECT
    CASE
        WHEN emp_no LIKE '_______1%' OR emp_no LIKE '_______3%' THEN '남'
        WHEN emp_no LIKE '_______2%' OR emp_no LIKE '_______4%' THEN '여'
    END AS 성별
