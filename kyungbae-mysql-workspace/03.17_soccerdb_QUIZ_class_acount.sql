...
SELECT
    team_id 팀ID
    , CONCAT(zip_code1, '-', zip_code2) 우편번호
FROM
    team;
...
SELECT
    COUNT(*) '인원 수'
    , SUM(height IS NOT NULL) '신장크기등록 인원수'
    , MAX(height) '최대 신장'
    , MIN(height) '최소 신장'
...
SELECT * FROM PLAYER;
SELECT
    IFNULL(nickname, player_name) 선수명
    , position 포지션
    , back_no 등번호
FROM
    player
...
FROM
    ...
    JOIN team t1 ON t1.team_id = s.hometeam_id
    JOIN team t2 ON t2.team_id = s.awayteam_id
WHERE
...
VALUES ((SELECT * FROM (SELECT player_id + 1 FROM player ORDER BY player_id DESC LIMIT 1)no_check), 
        '박주호', 
